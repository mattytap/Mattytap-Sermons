...
/**
 * The class that is used to initialize Mattytap Sermons.
 *
 * @author  Matt Fawcett (restoration); WP for Church (original)
 * @package SM/Core
 * @access  public
...
class SermonManager { // phpcs:ignore
	...
	/**
	 * Include Mattytap Sermons files.
	 *
	 * @return void
	 */
	private function _includes() {
		...
		include SM_PATH . 'includes/sm-core-functions.php'; // Core Mattytap Sermons functions.
		...
		include SM_PATH . 'includes/sm-views-functions.php'; // Sermon view counter.
		...
		if ( is_admin() ) {
			...
			include SM_PATH . 'includes/admin/sm-term-image-functions.php'; // Taxonomy term-image picker.
	...
	public function render_sermon_into_content( $post_ID = 0, $post = null, $skip_check = false ) {
		...
		// Direct $wpdb write bypasses wp_filter_post_kses; mirror its policy
		// here so non-unfiltered_html authors can't inject script/HTML via
		// any of the meta values concatenated into $content above.
	...
	/**
	 * Enqueue Mattytap Sermons scripts and styles
	 *
	 * @return void
	 */
	...
	/**
	 * Append the terms of Mattytap Sermons taxonomies to the list
	...
	public static function add_additional_sermon_classes( $classes, $class, $post_id ) {
		...
		/**
		 * Allows filtering of additional Mattytap Sermons classes
		 *
	...
	protected function _init_actions() {
		...
		// Add Mattytap Sermons image sizes.
		...
		add_action(
			'admin_init',
			function () {
				if ( ! isset( $_GET['page'], $_GET['doimport'] ) || 'sm-import-export' !== $_GET['page'] ) {
					return;
				}
				...
				$doimport = sanitize_key( wp_unslash( $_GET['doimport'] ) );
				check_admin_referer( 'sm_import_export_' . $doimport );
		...
		add_action(
			...
			function ( $value ) {
				if ( '' !== $value ) {
					...
					?>
					<div class="notice notice-success">
						<p>Removed <?php echo (int) $wpdb->rows_affected; ?> transient fields.</p>
					</div>
					<?php
				}
			...
			function ( $post_ID, $post, $update ) {
				...
				if ( $audio_url && ! $audio_id && true === sm_validate_public_url( $audio_url ) ) {
					...
					if ( ! is_wp_error( $response ) ) {
						...
						if ( '' !== $content_length ) {
							update_post_meta( $post_ID, '_wpfc_sermon_size', $content_length ?: 0 );
						}
					}
				}
			...
			function () {
				if ( ! SM_OB_ENABLED ) {
					...
					<div class="notice notice-wpfc-php notice-warning">
						<p>
							<?php
							// translators: %s: The plugin name. Effectively "<strong>Mattytap Sermons</strong>".
							echo wp_kses_post( wp_sprintf( __( '%s requires output buffering to be turned on to display content. It is currently off. Please enable it or contact your hosting provider for help. Most of plugin functionality will be disabled until output buffering is enabled.', 'mattytap-sermons' ), '<strong>' . __( 'Mattytap Sermons', 'mattytap-sermons' ) . '</strong>' ) );
							?>
						</p>
					</div>
					<?php
		...
		add_action(
			...
			function () {
				check_ajax_referer( 'sm_settings_nonce', 'nonce' );
				...
				$podcast_id = isset( $_POST['podcast_id'] ) ? absint( $_POST['podcast_id'] ) : 0;
